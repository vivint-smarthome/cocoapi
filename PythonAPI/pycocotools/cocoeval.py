...
class COCOeval:
    ...
    def _prepare(self):
        ...
        if p.useCats:
            ...
            if len(p.keepDtCatIds) > 0:
                dts = self.cocoDt.loadAnns(
                    self.cocoDt.getAnnIds(imgIds=p.imgIds, catIds=p.keepDtCatIds)
                )
            else:
            ...
            if len(p.keepGtCatIds) > 0:
                gts = self.cocoGt.loadAnns(
                    self.cocoGt.getAnnIds(imgIds=p.imgIds, catIds=p.keepGtCatIds)
                )
            else:
    ...
    def makeplot(
        self, recThrs, precisions, name, save_dir=None, confidence_indices=None
    ):
        ...
        for aidx in range(precisions.shape[2]):
            ...
            if save_dir is not None:
                ...
                ls.append("recThrs")
                df = pd.DataFrame(
                    np.concatenate((ps, np.expand_dims(recThrs, axis=0))).T, columns=ls
                )
    ...
    def analyze(self, save_to_dir=None, catIds=[]):
        ...
        for k, catId in enumerate(catIds):
            ...
            figures = self.makeplot(
                self.params.recThrs, ps[:, :, k, :, :], nm, save_dir=self.params.outDir
            )
    ...
    def analyze_threshold(self, confidence_threshold, catId):
        ...
        ce = COCOeval(
            self.cocoGt,
            self.cocoGt.loadRes(
                self.cocoDt.dataset["annotations"], threshold=confidence_threshold
            ),
            self.params.iouType,
        )
        prm = copy.deepcopy(self.params)
    ...
    def analyze_thresholds(self, save_to_dir=None, confidence_thresholds=[], catIds=[]):
        ...
        for k, catId in enumerate(catIds):
            ...
            for confidence_threshold in confidence_thresholds:
                confidence_indices[confidence_threshold] = self.analyze_threshold(
                    confidence_threshold, catId
                )
            figures = self.makeplot(
                self.params.recThrs,
                ps[:, :, k, :, :],
    ...
    def eval_custom(self, catIds=[], maxDets=[], ious=np.array([])):
        ...
        prm = copy.deepcopy(self.params)
        if len(maxDets) == 0:
            self.params.maxDets = [1, 5, 50]
        else:
            self.params.maxDets = sorted(maxDets)
        if len(catIds) == 0:
            catIds = sorted(self.cocoGt.getCatIds())
        self.params.catIds = catIds

        if len(ious) == 0:
            self.params.iouThrs = np.array([0.3, 0.35, 0.4, 0.45, 0.5])
        else:
    ...
    def summarize_2(self, is_print=True):
        ...
        def _summarizeDets():
            ...
            stats[1] = _summarize(
                1, iouThr=min(self.params.iouThrs), maxDets=self.params.maxDets[2]
            )
            stats[2] = _summarize(
                1, iouThr=max(self.params.iouThrs), maxDets=self.params.maxDets[2]
            )
            stats[3] = _summarize(1, areaRng="small", maxDets=self.params.maxDets[2])
            stats[4] = _summarize(1, areaRng="medium", maxDets=self.params.maxDets[2])
            stats[5] = _summarize(1, areaRng="large", maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        # area in image fraction - converting based on image size of 480x640
        self.areaRng = [
            [0.0 ** 2, 1.0 ** 2],
            [0.0 ** 2, 0.0577 ** 2],
            [0.0577 ** 2, 0.173 ** 2],
